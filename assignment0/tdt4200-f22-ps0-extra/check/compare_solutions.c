...
int main(int argc, char **argv) {
  ...
  fprintf(stderr,
          "There are %ld differences between the solution and the reference "
          "solution\n",
          errors);
...
void load_solution(char *filename, double *solution, int64_t N) {
  ...
  if (file_size != (N * sizeof(double))) {
    fprintf(stderr,
            "Expected solution size does not match actual solution size\n");
    exit(1);
  }
...
bool match_double(double d1, double d2) { return fabs(d1 - d2) <= epsilon; }

uint64_t compare_solutions(double *solution, double *other_solution,
                           int64_t N) {
